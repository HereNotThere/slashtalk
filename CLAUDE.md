...
**Session state is computed, never stored** (`sessions/state.ts`). `classifySessionState({heartbeatUpdatedAt, inTurn, lastTs})` → `BUSY | ACTIVE | IDLE | RECENT | ENDED`. Thresholds: heartbeat fresh < 30s, active < 30s since last event, recent < 1h. `in_turn` is the *only* reliable signal during a silent thinking block (tens of seconds with zero JSONL events) — by design it flips on at user prompt / queued command and off at assistant `stop_reason == "end_turn"`. Don't collapse this to "just use lastTs"; `specs/upload.spec.md` "Busy is computed, not observed" calls this out.
