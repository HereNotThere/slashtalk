...
`classifySessionState({heartbeatUpdatedAt, inTurn, lastTs})` → `BUSY | ACTIVE | IDLE | RECENT | ENDED`. Thresholds: heartbeat fresh < 30s, active < 10s since last event, recent < 1h. **`in_turn` is the only reliable BUSY signal during a silent thinking block** (zero JSONL events for tens of seconds). It flips on at user prompt or queued command and off at assistant `stop_reason == "end_turn"`. Don't collapse this to "just use lastTs" — `specs/upload.spec.md` "Busy is computed, not observed" calls this out.
