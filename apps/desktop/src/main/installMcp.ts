// Auto-install the Chatheads MCP server into local AI-client configs.
//
// Only Claude Code is truly automatable: we read-modify-write ~/.claude.json,
// touching only our own `chatheads` entry under `mcpServers`.
//
...
const MCP_URL =
  process.env["CHATHEADS_MCP_URL"] ?? "https://chatheads.onrender.com/mcp";
const MCP_KEY = "chatheads";
...
export async function install(target: McpTarget): Promise<TargetState> {
