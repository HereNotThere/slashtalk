...
A template is a *shape*, not a *quota*. Don't pad sections to fill the template — delete sections that don't apply. The shape is there so an agent reading the doc knows where to look; an empty section is worse than a missing one.
