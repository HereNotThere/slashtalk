...
- We **cannot** read repo contents server-side, even for repos the user has access to. The OAuth token covers identity, org listing, and `GET /repos/:owner/:name` access checks (which work for any repo the user can see, public or private).
- Repos are **claimed** on demand: the desktop app reads a local clone's `.git/config`, extracts `owner/name`, and POSTs `/api/me/repos { fullName }`. The server verifies with the user's stored OAuth token before inserting — see § Repo-claim verification.
...
- `404` → reject with **403 `no_access`**. The caller does not have visibility on GitHub.
