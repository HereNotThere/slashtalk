...
- OAuth `404` without a linked GitHub App → reject with **403 `github_app_required`** so the desktop can open `/auth/github-app`, which starts the GitHub App web authorization flow and stores the GitHub App user token from the callback. The explicit install/configure fallback is `/auth/github-app?install=1`; this keeps already-installed apps from dead-ending on GitHub's installation settings page without returning an OAuth `code`.
- No matching repository after GitHub App verification → reject with **403 `no_access`**. The caller does not have visibility on GitHub or the app installation does not include that repo; when the GitHub App path was used, the response includes `/auth/github-app?install=1` as a configure fallback.
