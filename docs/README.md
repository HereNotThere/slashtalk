...
| [`product-specs/`](product-specs/) | Public surface contracts: backend API, ingest protocol, WS message shapes. What we ship. | (no index yet — see contents) |
...
| `decisions/` *(planned)* | ADRs — append-only architecture decision records with `NNNN-slug.md` numbering. | — |
| `runbooks/` *(planned)* | Operational recovery procedures. Created on the second occurrence of a failure mode. | — |
